 ...
 * 通知文から `@ハンドル` を拾う。
 *
 * **確認済み (2026-08-05 / 実配信): 受信通知の `@名前` は実際のハンドルで、表示名ではない。**
 * その URL へ実際に遷移できる。日本語のハンドルもあり、以前の ASCII 限定の抽出では拾えなかった。
 *
 * **`HANDLE_PATTERN` は非 global なので、`text.match(HANDLE_PATTERN)` は最初の `@…` を採る。**
 * 文中に `@` が 2 つ以上あっても先頭が勝ち、それが送信元かどうかは判定しない。
 * 2026-08-06 に誤検知した送信側バナー (`この機会に、@<送信先> の…`) の `@` は送信先だった。
 * 現在は `selectors.ts` の `EXCLUDED_TEXT_PATTERNS` で既知の文言を除外しているが、
 * 未知の別形式まで除外できる保証はない。
 *
 * `extractRedirectEvent` の `??` の右側で、通知内のリンクから有効なチャンネル URL が取れない
 * ときだけ使うフォールバック。リンクを探す `selectors.ts` の `redirectNoticeChannelLink` は
 * `TODO(T1): 実 DOM で要確認` のままで、リンクが取れずにここへ落ちる実 DOM の形状は未観測。
 * 日本語を含むため文字種で絞れず、`{1,40}` と `TRAILING_PUNCTUATION` の末尾句読点除去で境界を推定している。
 */
