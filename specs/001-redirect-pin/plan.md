...
- **R6(新): 通知の `@名前` がハンドルとは限らない。** 表示名だった場合、そこから組み立てた
  `https://www.youtube.com/@<名前>` は**存在しない URL を指す**(AC2 が満たせない)。
  → 通知内にチャンネルへのリンクがあればそちらを優先する実装にしてあるが、リンクの有無は未確認。
  **投稿された URL が実際に飛ぶかの確認が要る。**
