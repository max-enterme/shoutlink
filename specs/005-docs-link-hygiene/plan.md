...
**T1(検査)・T2(違反の解消)・T4(登録)を 1 PR にまとめ、緑で着地させる。**

> **着手前の `test` は赤である。**`scripts/check-links.mjs` も
> `npm run check-links` も存在しないため。**T1+T2+T4 の PR で初めて緑になる。**
> T4 を後回しにすると `npm run check-links` が「script が無い」で落ち続ける。
...
**子プロセス方式にも `node:` 型定義が要る。**`@types/node` を devDependency に足し、
テスト冒頭に `/// <reference types="node" />` を書く(spec.md D3。実測で緑を確認)。
...
| `tests/check-links.test.ts` | **新規。**冒頭に `/// <reference types="node" />`。一時ディレクトリにフィクスチャを作り、**子プロセスで起動して exit code と出力を見る**。4 種それぞれの壊し方(AC2)と、③の**正例**を固定。あわせて `ci.yml` を `readFileSync` で読み `check-links` の文字列を含むことを固定(AC4) |
| `README.md` | 一覧表に `install.md` / `stability-research.md` / `004-t1-collect.md` を追加 |
| `package.json` | `"check-links": "node scripts/check-links.mjs"` と **devDependencies に `@types/node`**。**`test` script は `vitest run` のまま触らない**(CI へは `ci.yml` の独立ステップで足す) |
...
- 現在の main の実測値(この feature が守る基準):
  Markdown リンク **132 本(相対 85)・相対の切れ 0**(フェンス内 0)/ md 見出しアンカー **6 本**
  (相対 5 + index.html の絶対 1)/ 孤立 **1 本** / `docs/*.md` **9 本** /
  index.html の md リンクは**全部が絶対 URL・相対 0 本**、相対参照は `<img src>` 3 本のみ、
  Pages 絶対 URL は **`<meta content>` の 2 本だけ**(`href`/`src` には 0 本)
...
- **R4: 検査②の還元対象を広げすぎない。**
  ...
  **末尾 `/` の Pages URL(`og:url`)はディレクトリなので還元しない。**
- **R7: `docs/index.html` 側にコードフェンス相当の除外規則を持たせない。**
  現状 `<code>` 26 個の中に `href` / `src` は 1 本も無く、実害がゼロのため。
  006 でコード例に `href` を書くと誤検出するので、そのときに足す。
