...
- [ ] T9: 設定画面 — 有効化スイッチ / コメント用テンプレート + プレビュー / 辞書テーブルの**フラグ列とコメント返し用の自由文列**(**どちらも行編集で即保存**)/ **行を畳む表示**(既定は URL・呼び名・最終検知だけ。**畳んだ状態でも不整合が分かる**)/ 不整合の常時表示 / **投稿履歴の表に種別列**。自由文の 200 字検証・残り文字数・`{msg}` 不在の警告は **`commentTemplate` × `commentMessage` の組**で判定する(組を跨がない) (AC13 / AC16)。**文言はベタ書きの日本語**(003 spec.md D2 の決定により 002 は後に載るため、`_locales/` 化はしない)。**T14 のモックが確定してから着手する**(SPEC-OPS §10 / plan.md R9)  <!-- #33 -->
...
- [ ] T14: **辞書テーブルの畳む表示の GUI モックを作る**(SPEC-OPS §10 / AC13 / plan.md R9)。  <!-- #41 -->
      ...
      対象は **畳んだ状態 / 展開した状態 / どこを押すと開くか / 畳んだ状態での不整合の示し方**と、
      **2 つのスイッチの並び**(plan.md R8)。コメント用テンプレート欄とプレビューは対象外。
      **T9 の実装前に人間が確認して確定させる**
