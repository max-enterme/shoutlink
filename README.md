...
| アイコンと表記名をまとめて取得 | 辞書タブの左ペインにある**「アイコンと表記名をまとめて取得」**を押すと、アイコンか表記名のどちらかを控えていない行だけを取りに行きます (チャンネル ID は取り直しません)。**辞書タブを開いただけでは何も取りに行きません。**控えたアイコン・表記名は `chrome.storage.local` に保存され、次からは取りに行きません。**呼び名を付けていない行には、辞書タブの一覧で表記名をグレーで出します** (無ければ今までどおりハンドルだけ) |
...
への取得だけです (上の「チャンネル ID を控える」「アイコンと表記名をまとめて取得」。Cookie は付けません)。
