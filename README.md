...
1. 「リダイレクト返礼を使う」を**外す**
2. 「コメント返しを使う」も**外したままにする** (**既定 OFF**。①を外してもこちらは
   独立していて、ON なら投稿されます)
...
| アイコンと表記名をまとめて取得 | 辞書タブの左ペインにある**「アイコンと表記名を取得」**を押すと、アイコンか表記名のどちらかを控えていない行だけを取りに行きます (チャンネル ID は取り直しません)。**辞書タブを開いただけでは何も取りに行きません。**控えたアイコン・表記名は `chrome.storage.local` に保存され、次からは取りに行きません。**呼び名を付けていない行には、辞書タブの一覧で表記名をグレーで出します** (無ければ今までどおりハンドルだけ) |
