...
| 多重発火の抑止 | 同一送信元はクールダウン内 1 回まで (既定 6 時間)。**同じ配信の中でだけ**効き、投稿履歴に基づくのでリロードをまたいでも引き継がれる (**配信 ID を取れない画面では、前の起動から読み戻した記録に 6 時間の下限**をかけます → [docs/install.md](docs/install.md#上限が効かない場合)) |
