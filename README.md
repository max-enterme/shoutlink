...
| 多重発火の抑止 | **同じ配信で同じ相手には 1 回まで**。秒数では判定しない。配信が変われば再び 1 回投稿する。投稿履歴に基づくのでリロードをまたいでも引き継がれる (**配信 ID を取れない画面では、6 時間の窓**で判定します → [docs/install.md](docs/install.md#上限が効かない場合)) |
| 設定を試す | 設定画面の辞書の行を開くと「テスト送信」ボタンが出て、**開いている配信のチャットへ実際に投稿できる**。投稿履歴には残らないので何度でも試せる |
