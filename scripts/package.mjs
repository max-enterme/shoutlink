...
2026-08-23 の実配信で、自由文 ({msg}) 入りの文面とコメント返し (下記) は
検知 → 投稿 (→ 固定) まで通りました。まだ確かめられていないのは次の 3 点です。

・チャットを開き直したときに、過去のコメントへ一斉投稿しないか
・既定の固定モード ifEmpty の判定が、実際の固定バナーを拾うか
...
この ZIP は、デベロッパーモードで読み込む未署名の配布物です。
同じ拡張が Chrome ウェブストアの「限定公開」に出ています。そちらは署名されていて、
起動のたびの警告が出ず、新しい版が自動で届きます。

  https://chromewebstore.google.com/detail/kpjfmcppkncdmehdpgcochaanbmodaim

限定公開なのでストアの検索・一覧には出ません。この URL からだけ入ります。
以下は、それでも ZIP から入れる場合の手順です。

⚠️ ストア版とこの ZIP を同時に入れることはできません。
   0.3.0 以降のビルドはストア版と同じ拡張 ID です。どちらかを削除してください。
   削除すると、その拡張に保存されていた設定・呼び名の辞書・投稿履歴も一緒に消えます。
   引き継ぎたい場合は、消す前に設定ページの開発者ツール (F12) の Console で

     Promise.all([chrome.storage.local.get(null), chrome.storage.sync.get(null)])
       .then(([local, sync]) => copy(JSON.stringify({ local, sync })))

   を実行して保存し、入れ替えたあとに set() で書き戻してください。
   投稿履歴も一緒に書き戻すこと。捨てると再投稿の抑止が白紙になり、
   同じ配信で既に返礼した相手へもう一度投稿することがあります。
...
・同じ配信・同じ人には 1 回まで / 投稿の間隔は最低 5 秒。
  この 2 つは設定では変えられません。件数の上限はありません。
...
⚠️ コメント返しは 2026-08-23 の実配信で通しました。登録した人のコメントで投稿されること、
   固定されないこと、同じ配信で 2 回目が出ないこと、自分の投稿を引き金に再投稿しないことは
   確かめています。チャットを開き直したときに過去のコメントへ一斉投稿しないかだけが未確認です。
   試すときは必ず限定公開のテスト配信で。
...
・自動更新はされません。直しても、自分で入れ替えるまで新しい版は届きません。
  自動更新が要るならウェブストア版へ (上記)。

■ 新しい版に入れ替えるとき

ZIP どうしの入れ替えなら、設定・呼び名の辞書・投稿履歴は引き継がれます
(拡張 ID が同じままのため)。書き写す必要はありません。

⚠️ ただし 0.2.0 以前から 0.3.0 へ上げるときだけは拡張 ID が変わるので、
   引き継がれません。この場合も先に書き出してください (冒頭)。
...
⚠️ ウェブストア版へ移るときは話が別です。拡張を削除した時点で保存も消えるので、
   先に書き出してください (冒頭の「ふつうはウェブストアから入れてください」)。
`
