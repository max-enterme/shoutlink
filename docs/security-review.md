...
> 回帰テストを足して 118 tests green。**S3 / S4 / S6 / S7 は未対応。**
...
| 3 | `decodeURIComponent` の例外でパイプラインが落ちた | **解消。**`safeDecode` が握る。ただし握った結果の検証が無い → S4 | `tests/detector.test.ts:43` |
...
### S3 (中) 第三者が制御する表示名が、そのまま自分名義の投稿に載る

`extractRedirectEvent` の `name` は
`getRedirectNoticeChannelName(el) || textOf(link) || handleFromChannelUrl(url)`。
前 2 者は**通知 DOM のテキストそのまま**で、YouTube のチャンネル表示名は第三者が自由に決められる。
`composer` は制御文字と空白を潰すだけで、内容の検査はしない。

jsdom で確認:

```
表示名: 見てね https://evil.example/free-money
投稿文: 見てね https://evil.example/free-moneyさんからリダイレクトありがとうございます! https://www.youtube.com/@evil
```

**配信者が自分の名義で、第三者の指定した文字列と URL を自分の視聴者へ流す**ことになる。
リダイレクトを送れるのは登録者 1,000 人以上のチャンネルに限られるので敷居は高いが、
「相手からの厚意」を前提にした設計であって、防御にはなっていない。

あわせて、`redirectNoticeChannelName` の候補に `yt-formatted-string#text` があるため、
通知の**本文全体**が表示名として拾われうる(文面が壊れるだけの実害の小さい経路だが同根)。

**対処案:** 表示名に長さ上限(例 40 文字)を掛け、URL 風のトークン
(`http://` / `https://` / `www.` / `://`)を落とす。`{url}` は正規化済みの値だけを使う。

### S4 (中) `normalizeChannelUrl` が「正規化済み URL」を保証していない

`safeDecode` はパスから切り出した後に呼ばれるため、**パーセントエンコードで潰していた
空白や区切り文字が復活する。**戻り値は dedupe の鍵 / 辞書の鍵 / 投稿文の `{url}` に使われる。
...
`textContent` にしか流れないので XSS にはならないが、**投稿文に任意の URL を混ぜられる**点で
S3 と同じ実害につながる。また同じチャンネルが表記違いで別エントリとして辞書に増える。

**対処案:** デコード後のハンドルを `[^\s@/?#]{1,40}` で再検証し、通らなければ
デコード前の値を使う(または `null` を返す)。
...
- ❌ S3: 表示名に URL が入っている場合(未カバー)
- ❌ S4: `@a%20https:%2F%2F…` 形式(未カバー。既存は「壊れたエンコードで落ちない」までしか見ていない)
...
| S3 / S4 | 第三者が決める表示名と `safeDecode` 後の URL の検証 | ❌ 未対応 |
